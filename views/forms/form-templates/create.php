...
if (count($selectgroups) > 0) {
	foreach ($selectgroups as $selectcolumn => $selectcolumndata) {
		...
		while ($currentprocessingnumber <= $maxlevel) {
			$optionsgroup[$currentprocessingnumber] = $currentprocessingnumber;
...
$orderoptions = array();
if ($orderenabled) {
	$databaseorderquery = "select {$ordercolumn} from {$tablename}";
	...
	while ($currentorder <= $ordermax) {
		if (in_array($currentorder, $dborderdataresult)) {
...
&& $_POST['secureInputField'] == md5(session_id())) {
	...
	$query = $query.$querycolumns.$queryvalues;
	...
	if (count($errors)>0) {
		...
		$result = array(
			"status"=>"success",
			"message"=>"{$nodename} creada exitosamente!"
		);
		$return = json_encode($result);
		header('Content-Type: application/json');
		echo $return;
...
<div class="modal-header">
	...
	<h4 class="modal-title">Crear <?=$nodename?></h4>
</div>
<div class="modal-body">
	<form id="<?=$formid?>" class="panel-body" role="form" action="<?=$formcreateaction?>">
			...
			<div class="form-group">
			    ...
			    switch ($columndata['input-type']) {
					case 'text': ?>
						...
						<input type="text" name="<?=$columnname?>" class="<?=$columndata['input-class']?>" id="<?=$columnname?>" placeholder="<?=$columndata['placeholder']?>" value="">
				<?php	break;
					case 'radio': ?>
						...
						?>
								<label class="<?=$columndata['input-class']?>"><input type="radio" value="<?=$inputvalue?>" name="<?=$columnname?>"><?=$inputlabel?></label>
						<?php	
							}
						?>
				<?php	break;
				...
				<?php	break;
					case 'textarea': ?>
						<label for="<?=$columnname?>"><?=$columndata['label']?>:</label>
						<textarea name="<?=$columnname?>" class="<?=$columndata['input-class']?>" id="<?=$columnname?>" placeholder="<?=$columndata['placeholder']?>" rows="5"></textarea>
				<?php	break;
					default: ?>
